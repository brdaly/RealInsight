...
const LINKING = new Set([
  ...
  "hold", "holds", "fit", "fits", "house", "houses", "take", "takes",
...
/**
 * Shaped like a linking word, but additive rather than negating.
 *
 * "Not only 3 beds, but also a studio" states the bedrooms and then exceeds
 * them. Allowing "only" through would discard a fact the text is emphasising.
 */
const NOT_LINKING = new Set(["only"]);
...
function isNegated(text, charStart) {
  ...
  return words.every((word) => {
    ...
    return !NOT_LINKING.has(lower) && (LINKING.has(lower) || ADVERB.test(lower));
  });
}
