 ...
 *
 * The gap between the negator and the match is horizontal space, or a single
 * hyphen with nothing around it. Both exclusions are load-bearing:
 *
 *   "HOA: No\n3 beds | 2 baths"   a line break ends the field. The "No" answers
 *                                 HOA, not the bedroom count, and allowing \s
 *                                 here lost both facts on ordinary spec-sheet
 *                                 copy — the same silent loss, one character
 *                                 further out.
 *   "HOA: no - 3 beds"            a spaced hyphen separates items. Only an
 *                                 intra-word hyphen binds, which is what makes
 *                                 "not-active" and "no-longer active" negate
 *                                 exactly as their spaced spellings do.
 */
const GOVERNING_NEGATION =
  /(?:^|[^\p{L}\p{N}])(?:no|not|never|without|excluding|lacks?|lacking|missing|\p{L}+n't)(?:(?:-|[^\S\r\n]+)(?:longer|more|any|a|an|the))?(?:-|[^\S\r\n]+)?$/iu;
