...
const LINKING = new Set([
  ...
  "hold", "holds", "fit", "fits", "house", "houses",
  "accommodate", "accommodates", "sleep", "sleeps",
  "support", "supports", "allow", "allows", "permit", "permits",
 ...
 * Case-insensitive because listing copy is often set in capitals, and the same
 * sentence must not read one way in "comfortably" and another in "COMFORTABLY".
 *
 * The shape is not the part of speech, though: plenty of adjectives end in -ly,
 * and in "not a costly 3 bed home" the negation governs the price, not the
 * bedroom count. The ones a listing is likely to use are named below, because
 * nothing about the spelling separates them.
 */
const ADVERB = /^\p{L}+ly$/iu;

/** Ends in -ly and is an adjective, so a negator before it governs it. */
const ADJECTIVE_IN_LY = new Set([
  "costly", "lovely", "friendly", "homely", "early", "timely", "lively", "orderly",
  "elderly", "stately", "leisurely", "worldly", "shapely", "unsightly", "unruly",
  "chilly", "hilly", "curly", "ugly", "silly", "holy", "jolly", "bubbly", "crumbly",
  "deadly", "comely", "neighbourly", "neighborly", "motherly", "scholarly",
]);

/**
 * "not only X, but also Y" states X and then exceeds it, so the negator does
 * not withhold the number after it.
 *
 * Scoped to that collocation rather than to the word. "only" reaches a fact
 * perfectly well after a different negator — "the home never had only 3 beds"
 * denies the three — so excluding it everywhere turned one idiom into a rule
 * that suppressed real negations.
 */
const ADDITIVE_NEGATOR = /^(?:not|\p{L}+n['\u2019]?t)$/iu;
...
function isNegated(text, charStart) {
  ...
  for (let match = NEGATOR.exec(before); match !== null; match = NEGATOR.exec(before)) {
    ...
    negator = match[0].replace(/^[^\p{L}\p{N}]+/u, "");
  ...
  if (words.length === 1 && words[0].toLowerCase() === "only" && ADDITIVE_NEGATOR.test(negator)) return false;

  return words.every((word) => {
    const lower = word.toLowerCase();
    return !ADJECTIVE_IN_LY.has(lower) && (LINKING.has(lower) || ADVERB.test(lower));
  });
}
